...
The Admin FCM API allows you to send messages to individual devices by specifying the target device's
`Firebase Installation ID (FID) <https://firebase.google.com/docs/cloud-messaging/android/get-started#access-firebase-installation-id>`_.
...
Before Firebase Installation IDs, messages were targeted with a registration token, a string
`generated by the client FCM SDKs <https://firebase.google.com/docs/cloud-messaging/get-started>`_
...
.. note::
    The FCM API `deprecated <https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages>`_
    the ``token`` field in favor of the ``fid`` field. Until Google decommissions it, the ``token``
    field remains fully supported and accepts Firebase Installation IDs as well, so no immediate
    action is required. Use ``withFid()`` once your clients provide Firebase Installation IDs.

    A message can only have one target: setting a FID on a message that has a registration token
    (or a topic, or a condition) replaces the previous target.
...
To send a message to multiple Firebase Installation IDs, pass them as a
``Kreait\Firebase\Messaging\FirebaseInstallationIds`` instance. Plain strings and arrays of strings
are always treated as registration tokens.
...
.. code-block:: php
    ...
    /** @var Kreait\Firebase\Messaging\MulticastSendReport $sendReport **/
    $sendReport = $messaging->sendMulticast($message, $fids);
...
.. code-block:: php
    ...
    // When the message was sent to Firebase Installation IDs, the equivalent methods are
    $successfulTargets = $report->validFids(); // string[]
    $unknownTargets = $report->unknownFids(); // string[]
